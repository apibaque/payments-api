...
public interface PaymentOrderApi {
    ...
    @ApiOperation(value = "Get payment for an order", nickname = "getPayment", notes = "A `due payment` is a scheduled operation on an `account`. It can be updated or deleted until its' due date. Once the payment date is reached, the due payments becomes a payment. When the payment is processed, it becomes a `transaction` on that account.", response = PaymentOrderResponse.class, tags={ "Payments", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Request succeeded", response = PaymentOrderResponse.class),
    ...
    ResponseEntity<PaymentOrderResponse> getPayment(@Pattern(regexp="^\\w{8}-\\w{4}-\\w{4}-\\w{4}-\\w{12}$") @Size(min=36,max=36) @ApiParam(value = "Order id Number of current payment",required=true) @PathVariable("orderId") String orderId,@ApiParam(value = "The JWT Token generated from Get API Token" ) @RequestHeader(value="x-dnbapi-jwt", required=false) String xDnbapiJwt,@ApiParam(value = "The API key from your app page in DNB Developer" ) @RequestHeader(value="x-api-key", required=false) String xApiKey);


    @ApiOperation(value = "Initiate payment transaction", nickname = "paymentOrder", notes = "Initiate a domestic payment going directly from client account with  order Id or message. The recieving account can be in whichever bank.  Accepts a ```paymentOrderRequest``` POST, and returns HTTP 201 with the ```orderId``` of the successfully created ```payment```.  ***Note**: As our sandbox is currently static, initiating payments unfortuntately won't reflect in neither the balance nor latest transactions of the account. While we are working hard to get you a dynamic sandbox, you may in the meanwhile look at it as finally having that infinite account never getting affected by your purchases.*", response = PaymentOrderResponse.class, tags={ "Payments", })
    @ApiResponses(value = { 
